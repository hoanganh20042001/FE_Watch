...
export default function DashboardContent() {
    ...
    React.useEffect(() => {
        ...
        //         console.log(error);
        //     });
        tablePay().then(data=>setMems(data))
        console.log(mems)
    }, [render]);
    ...
    const rows = mems.map((element) => (
        <tr key={element.name}>
            ...
            <td> <Text>{moment(element.day).format("DD/MM/YYYY ")}</Text></td>
            <td>{element.totalCost?element.totalCost:0}</td>
            ...
            <Grid style={{}}>
                <Grid.Col className="">
                    <Group position="center" style={{ paddingBottom: "2%", margin: "2% 5%", borderBottom: "1px solid #000" }}>
                        <Title order={1} >Thông tin doanh thu</Title>
                    </Group>
